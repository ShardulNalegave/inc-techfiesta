...
export class RoadController {
  ...
  async runDijkstraSafest(startId: number, endId: number): Promise<any> {
    const query = `
      MATCH (start:Road {road_id: $startId}), (end:Road {road_id: $endId})
  ...
  async runDijkstraLeastCrime(startId: number, endId: number): Promise<any> {
    const query = `
    MATCH (start:Road {road_id: $startId}), (end:Road {road_id: $endId})
    MATCH path = shortestPath((start)-[:CONNECTED_TO*]-(end))
    WHERE all(r IN relationships(path) WHERE r.crime_weight IS NOT NULL) // Ensure distance property exists
    WITH path, reduce(totalCost = 0, r IN relationships(path) | totalCost + r.crime_weight) AS totalCost, start, end
    UNWIND nodes(path) AS node
    RETURN id(node) AS nodeId, node.startPoint AS position, id(start) AS sourceNode, id(end) AS targetNode, totalCost;
  `;
    return this.session.run(query, { startId, endId });
  ...
  async runDijkstraStreetLight(startId: number, endId: number): Promise<any> {
    const query = `
    MATCH (start:Road {road_id: $startId}), (end:Road {road_id: $endId})
    MATCH path = shortestPath((start)-[:CONNECTED_TO*]-(end))
    WHERE all(r IN relationships(path) WHERE r.street_light_weight IS NOT NULL) // Ensure distance property exists
    WITH path, reduce(totalCost = 0, r IN relationships(path) | totalCost + r.street_light_weight) AS totalCost, start, end
    UNWIND nodes(path) AS node
    RETURN id(node) AS nodeId, node.startPoint AS position, id(start) AS sourceNode, id(end) AS targetNode, totalCost;
  `;
    return this.session.run(query, { startId, endId });
  ...
  async runDijkstraLeastAccident(startId: number, endId: number): Promise<any> {
    const query = `
    MATCH (start:Road {road_id: $startId}), (end:Road {road_id: $endId})
    MATCH path = shortestPath((start)-[:CONNECTED_TO*]-(end))
    WHERE all(r IN relationships(path) WHERE r.traffic_weight IS NOT NULL) // Ensure distance property exists
    WITH path, reduce(totalCost = 0, r IN relationships(path) | totalCost + r.traffic_weight) AS totalCost, start, end
    UNWIND nodes(path) AS node
    RETURN id(node) AS nodeId, node.startPoint AS position, id(start) AS sourceNode, id(end) AS targetNode, totalCost;
  `;
    return this.session.run(query, { startId, endId });
